...
class Node
{
    ...
    Node(Costpath y);
    Node(int x, Costpath y);
};

Node::Node(Costpath y) : data(y) {
}

Node::Node(int x, Costpath y) : degree(x), data(y) {
}

  
Node* newNode(Costpath key) 
{ 
...
list<Node*> insert(list<Node*> _head, Costpath key) 
{ 
...
{ 
    ...
    while (it != _heap.end()) 
    { 
        if ((*it)->data.cost < temp->data.cost) 
            temp = *it; 
        it++; 
    } 
    return temp; 
