...
bool CarSetupComHandle::CheckonMap (const std::string id, int seq, double x, double y) {
    if (map.count(id) < 1) {
        ...
        //std::cout << "carsetupcomhandle " << "X: " << x << ", Y: " << y << std::endl;
        ...
        //std::cout << "carsetupcomhandle " << "FX: " << fx << ", FY: " << fy << std::endl;
        //if (fx < 0 || fy < 0)
        //    return false;
        //int j = (int)(fx/res);
        //int i = (int)(fy/res);
        ...
        int i =100+ (int)(y/res);
        int w = mp -> info.width;
        // DEEEEEEEEBBBBBUUUUUUUUGGGGGGG
        /* 
        for (int kl = 0; kl < mp->info.height; kl++){
        for (int ij = 0; ij < mp->info.width; ij++){
            std::cout << (int)(mp->data[w*kl+ij]) << " ";
        }
        std::cout << std::endl;
        std::cout << std::endl;
        }
        std::cout << "BUG DEBUG" << std::endl;
        */ 
        if (i < mp->info.height && j < mp->info.width ) {
            if (mp->data[w*i+j] == 0){
                //std::cout << "YES! UNOCCUPIED" << std::endl;
                return true;
            }
            else {
                //std::cout << "NO.. OCCUPIED" << std::endl;
                return false;
            }
        }
        else{
...
bool CarSetupComHandle::isStateValid (std::string id, int seq, const ob::StateSpacePtr &space, const ob::State *state) {
    ...
    //std::cout << "ISSTATEVALID WORKING....." << std::endl;
